...
def run_logers(prm_logname,prm_logname_debug,prm_logname_error):
    f = logging.Formatter(fmt=u'%(filename)s[LINE:%(lineno)d]# %(levelname)-8s [%(asctime)s]  %(message)s',
        datefmt="%Y-%m-%d %H:%M:%S")
    handlers = [
        logging.handlers.RotatingFileHandler(prm_logname, encoding='utf8', maxBytes=100000000, backupCount=100),
        logging.StreamHandler(),
        logging.handlers.RotatingFileHandler(prm_logname_debug, encoding='utf8', maxBytes=100000000, backupCount=100),
        logging.handlers.RotatingFileHandler(prm_logname_error, encoding='utf8', maxBytes=100000000, backupCount=100)
    ]
